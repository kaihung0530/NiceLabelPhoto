...
每天固定時間,bot 自動推一則「今日待辦 + 今日行程」給你。

### 4-1 前提
- 你至少要**傳過一句訊息**給 bot(它才記得住要推給誰)
- LINE 免費方案主動推播每月 500 則;每天 1 則約 30 則/月,足夠

### 4-2 設定 Cron 排程
1. Cloudflare → 你的 Worker → 上方 **設定** → 找 **觸發事件(Triggers)** 區塊(或上方就有「觸發事件」)
2. **Cron 觸發程序 → 新增 Cron 觸發程序**
3. 填 Cron 運算式(注意:用的是 **UTC 時間**,台灣要減 8 小時):
...
### 4-3 測試 ✅
- 傳 LINE:`今日摘要` → 應立即收到一則推播(這就是每天會自動收到的內容)
- 之後每天到設定時間就會自動送來
