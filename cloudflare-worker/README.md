...
| `刪行程 2` | 刪除清單第 2 筆 |
| `房屋 : 高雄 三民 左營 透天 2000萬` | 新增一筆房屋買賣監看(縣市必填,行政區可多個,種別/房數/總價可省略) |
| `房屋清單` | 列出目前監看條件(含編號) |
| `刪房屋 1` | 刪除第 1 筆監看 |
| `找房` | 立即查詢所有監看條件目前符合的物件 |
| `今日摘要` | 立即推播今日待辦+行程 |
| 其他文字 | 回覆使用說明 |
...
設定想買的條件後,Worker 會在每次 Cron 排程時去 **591 售屋(sale.591.com.tw)** 抓該縣市銷售中的物件,依你的條件過濾,**出現沒通知過的新物件才主動用 LINE 推播**(第一次只記錄目前物件、不灌爆你,之後才通知新的)。

- 指令格式:`房屋 : 縣市 [行政區…] [種別] [N房] [總價萬]`
  - 例:`房屋 : 高雄 三民 左營 透天 2000萬`(高雄市、三民或左營、透天、總價 2000 萬以內)
  - 例:`房屋 : 宜蘭 3房 1500萬`
  - 行政區可寫多個(以 OR 比對地址);種別支援 透天／公寓／大樓／華廈／別墅／套房／店面 等
  - 支援縣市:台北、新北、基隆、桃園、新竹、苗栗、台中、彰化、南投、雲林、嘉義、台南、高雄、屏東、宜蘭、台東、花蓮、澎湖、金門、連江
- 查詢是「縣市層級」抓回,再由 Worker 本地過濾行政區/種別/房數/總價,因此不需維護 591 內部區碼。
- 建議把 Cron 設成每數小時一次(例如 `0 */3 * * *`)以便較即時地發現新物件;早安摘要一天仍只發一次,不會被洗版。

> ⚠️ **關於 591 的穩定性**:591 有 bot 防護。程式已用「先取首頁 cookie + csrf-token 再打清單 API」的方式擬真請求,但 591 隨時可能改版或加強防護導致抓不到(此時 `找房` 會回報查詢失敗、Cron 會靜默略過,不影響待辦/行程功能)。抓資料的部分集中在 `fetch591Sale()` 一個函式,若之後 591 被擋死,只要改寫它(例如改抓你提供的 RSS/查詢網址)即可,其餘指令與通知邏輯完全不用動。
...
想同時勤跑房屋新著檢查,可把 Cron 改成較密集的排程(例如 `0 */3 * * *`,每 3 小時一次)。早安摘要有「一天只發一次」的防重複機制,所以就算 Cron 每幾小時觸發一次,也只有房屋檢查會每次跑,早安摘要仍是一天一則。
